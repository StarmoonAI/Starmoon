...
export const createAccessToken = (
                ...
                if (expireDays) {
                                ...
                                toEncode.exp = Math.floor(
                                                expire.getTime() / 1000,
                                ); // JWT expects 'exp' in seconds since epoch
                }

                // Convert created_time to ISO format string
                if (toEncode.created_time) {
                                toEncode.created_time = new Date(
                                                toEncode.created_time,
                                ).toISOString();
                }
...
export const getCreditsRemaining = (user: IUser): number => {
                ...
                const remainingCredits = Math.round(
                                INITIAL_CREDITS - usedCredits,
                );
