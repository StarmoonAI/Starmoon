...
const getCardsData = (
        ...
        for (const key of Object.keys(curAvg)) {
                if (prevAvg[key] !== undefined) {
                        const change =
                                ((curAvg[key] - prevAvg[key]) / prevAvg[key]) *
                                100;
                        changes[key] = change;
                }
        }
...
const getBarData = (
        ...
        const barData = curAvgTopN.map(([emotion, currentPeriodValue]) => {
                ...
                return {
                        emotion,
                        [currentPeriodLabel]:
                                roundDecimal(currentPeriodValue) ?? 0, // Ensure this is a number
                        [previousPeriodLabel]:
                                roundDecimal(prevPeriodValue) ?? 0, // Ensure this is a number
                };
        });
...
export const getPieLinedata = (
        ...
        data.forEach((entry) => {
                ...
                if (entry.metadata && entry.metadata.scores) {
                        Object.keys(entry.metadata.scores).forEach(
                                (emotion) => {
                                        const score =
                                                entry.metadata.scores[emotion];
        ...
        Object.keys(dailyScores).forEach((date) => {
                ...
                const totalSum =
                        positiveAverage + negativeAverage + neutralAverage;
