...
export const processData = async (
                ...
                const currentPeriodData = filterDataByDate(
                                rawData,
                                currentPeriod,
                );
...
const getBarData = (
                ...
                const barData = curAvgTopN.map(
                                ([emotion, currentPeriodValue]) => {
...
const getSortedAvgData = (
                ...
                const prevAvgSorted = Object.fromEntries(
                                Object.entries(prevAvg).sort(
                                                ([, a], [, b]) => b - a,
                                ),
                );

                const curAvgSorted = Object.fromEntries(
                                Object.entries(curAvg).sort(
                                                ([, a], [, b]) => b - a,
                                ),
                );
                ...
                const sum = Object.values(curAvgSorted).reduce(
                                (a, b) => a + b,
                                0,
                );
                // console.log("sum: ", sum);
...
export const getPieLinedata = (
                ...
                data.forEach((entry) => {
                                ...
                                if (entry.metadata && entry.metadata.scores) {
                                                ...
                                                dailyScores[date].positive.push(
                                                                0,
                                                );
                                                dailyScores[date].negative.push(
                                                                0,
                                                );
                                                dailyScores[date].neutral.push(
                                                                0,
                                                );
                                }
                });
                ...
                Object.keys(dailyScores).forEach((date) => {
                                const positiveScores =
                                                dailyScores[date].positive;
                                const negativeScores =
                                                dailyScores[date].negative;
                                ...
                                const normalizedPositive =
                                                positiveAverage / totalSum ||
                                                null;
                                const normalizedNegative =
                                                negativeAverage / totalSum ||
                                                null;
                                const normalizedNeutral =
                                                neutralAverage / totalSum ||
                                                null;

                                lineData[0].data.push({
                                                x: date,
                                                y: normalizedNegative,
                                });
